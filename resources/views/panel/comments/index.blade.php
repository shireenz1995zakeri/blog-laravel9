<x-panel-layout>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr role="row">
                        ...
                        <td>
                            <a href="" class="item-delete mlg-15" title="حذف" style="color: red;" onclick="destroyComment(event,'{{$comment->id}}')">حذف</a>

                            <a href="show-comment.html" target="_blank" class="item-eye mlg-15" title="مشاهده" style="color: lightgrey;">مشاهده</a>
                            @if($comment->is_approved)
                            <a href="{{ route('comments.update',$comment->id) }}"  class="item-reject mlg-15" title="رد" onclick=" updateComment(event,'{{$comment->id}}')">رد</a>
                            @else
                            <a href="{{ route('comments.update',$comment->id) }}" class="item-confirm mlg-15" title="تایید" onclick=" updateComment(event,'{{$comment->id}}')">تایید</a>
                            @endif
    ...
    <script>
                ...
                function destroyComment(event, id) {
                    event.preventDefault();
                    Swal.fire({
                        title: 'آیا مطمعن هستید می خواهید این نظر را حدف کنید؟',
                        text: "",
                        icon: 'warning',
